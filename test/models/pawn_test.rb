...
class PawnTest < ActiveSupport::TestCase
  def setup
    ...
    @second_white_pawn = Pawn.create(color: 'white', x_position: 3, y_position: 3)
    @first_black_pawn = Pawn.create(color: 'black', x_position: 0, y_position: 1)
    @second_black_pawn = Pawn.create(color: 'black', x_position: 0, y_position: 2)
  end
  ...
  test "Invalid backward move" do
    ...
    assert_equal false, @second_white_pawn.valid_move?(3, 4)
    assert_equal false, @first_black_pawn.valid_move?(0, 0)
    assert_equal false, @second_black_pawn.valid_move?(0, 1)
  end

  test "After white first move, 1 space foward" do
    assert_equal true, @second_white_pawn.valid_move?(3, 2)
  end

  test "Invalid move, moved too many steps foward" do
    assert_equal false, @second_white_pawn.valid_move?(3, 1)
    assert_equal false, @second_black_pawn.valid_move?(0, 4)
  end
  ...
  test "After black first move, 1 space foward" do
    assert_equal true, @second_black_pawn.valid_move?(0, 3)
  end
end
